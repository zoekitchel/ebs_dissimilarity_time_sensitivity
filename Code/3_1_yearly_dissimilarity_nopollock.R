###########################
#This script calculates annual average pairwise dissimilarity, LEAVING OUT POLLOCK
...
#exclude pollock
EBS.domains.r.nopollock <- EBS.domains.r.full[accepted_name != "Gadus chalcogrammus"]

rm(EBS.domains.r.full)

#check that there is one species record for each haul id
stopifnot(
  nrow(unique(EBS.domains.r.nopollock[,.(accepted_name, haul_id)])) == nrow(EBS.domains.r.nopollock)
)

#calculate avg # haul ids per year domain and across all domains
EBS.domains.r.nopollock[, hauls_per_year := uniqueN(haul_id), .(year)][, avg_hauls_per_year := median(hauls_per_year)]
EBS.domains.r.nopollock[, hauls_per_year_bydomain := uniqueN(haul_id), .(year, domain)][, avg_hauls_per_year_bydomain := median(hauls_per_year_bydomain), domain]


#list years
EBS.domains.r.nopollock[, year:= as.numeric(year)] #make numeric
setorder(EBS.domains.r.nopollock,  year)
years <- unique(EBS.domains.r.nopollock[, year])

#haul id keys
haul_ids <- unique(EBS.domains.r.nopollock[, haul_id])
haul_ids_key <- data.table(haul_id = haul_ids,  key_ID = seq(1, length(haul_ids),  by = 1))


#convert haul_ids to numeric key_ids
EBS.domains.r.nopollock <- EBS.domains.r.nopollock[haul_ids_key,  on = "haul_id"]
...
for (j in 1:length(years)) {
  EBS.singleyear <- EBS.domains.r.nopollock[year == years[j], ]
...
###################
domains <- levels(as.factor(EBS.domains.r.nopollock[,domain]))
...
for (i in 1:length(domains)) {
  EBS.single.domain <- EBS.domains.r.nopollock[domain == domains[i], ]
  for (j in 1:length(years)) {
...
EBS.distances_dissimilarities_allyears_nopollock <- rbind(EBS.distances_dissimilarities_allyears_fullEBS.r, EBS.distances_dissimilarities_allyears_by_domain.r)
...
#and again with no pollock
EBS.domains.r.nopollock <- ggplot(EBS.distances_dissimilarities_allyears_nopollock) +
  geom_point(aes(x = year, y = bray_curtis_dissimilarity_total_mean, color = domain)) +
  labs(color = "Domain", x = "Year", y = "β diversity") +
...
saveRDS(EBS.domains.r.nopollock, file.path("Figures","Supplement","Nopollock","EBS.domains.r.nopollock.Rds"))
...
R2_full <- round(summary(lm(bray_curtis_dissimilarity_total_mean~year, data = EBS.distances_dissimilarities_allyears[domain == "Full",]))$adj.r.squared,2)
R2_inner <- round(summary(lm(bray_curtis_dissimilarity_total_mean~year, data = EBS.distances_dissimilarities_allyears[domain == "Inner",]))$adj.r.squared,2)
R2_middle <- round(summary(lm(bray_curtis_dissimilarity_total_mean~year, data = EBS.distances_dissimilarities_allyears[domain == "Middle",]))$adj.r.squared,2)
R2_outer <- round(summary(lm(bray_curtis_dissimilarity_total_mean~year, data = EBS.distances_dissimilarities_allyears[domain == "Outer",]))$adj.r.squared,2)
...
p_value_full <- round(summary(lm(bray_curtis_dissimilarity_total_mean~year, data = EBS.distances_dissimilarities_allyears[domain == "Full",]))$coefficients[2,4],2)
p_value_inner <- round(summary(lm(bray_curtis_dissimilarity_total_mean~year, data = EBS.distances_dissimilarities_allyears[domain == "Inner",]))$coefficients[2,4],2)
p_value_middle <- round(summary(lm(bray_curtis_dissimilarity_total_mean~year, data = EBS.distances_dissimilarities_allyears[domain == "Middle",]))$coefficients[2,4],2)
p_value_outer <- round(summary(lm(bray_curtis_dissimilarity_total_mean~year, data = EBS.distances_dissimilarities_allyears[domain == "Outer",]))$coefficients[2,4],2)
...
year_beta_bydomain_annotate_nopollock <- ggdraw(xlim = c(0,10), ylim = c(0,8)) +
  draw_plot(EBS.domains.r.nopollock, x = 0, y = 0, width = 10, height = 8) +
...
ggsave(year_beta_bydomain_annotate_nopollock, path = file.path("Figures","Supplement","Nopollock")"Nopollock"), filename = "year_beta_bydomain_annotate_nopollock.jpg", height = 6, width = 8, unit = "in")
...
year_beta_jaccard_bydomain_jaccard <- ggplot(EBS.distances_dissimilarities_allyears) +
  geom_point(aes(x = year, y = jaccard_dissimilarity_total_mean, color = domain)) +
...
saveRDS(year_beta_jaccard_bydomain_jaccard, file.path("Figures","Supplement","Jaccard","year_beta_jaccard_bydomain_jaccard.Rds"))
...
#place model values on plot
year_beta_jaccard_bydomain_annotate_nopollock <- ggdraw(xlim = c(0,10), ylim = c(0,8)) +
...
ggsave(year_beta_jaccard_bydomain_annotate_nopollock, path = file.path("Figures","Supplement","Nopollock")"Jaccard_nopollock"), filename = "year_beta_jaccard_bydomain_annotate_nopollock.jpg", height = 6, width = 8, unit = "in")


#####################
##Save
#####################
fwrite(EBS.distances_dissimilarities_allyears_nopollock, file.path("Output","EBS.distances_dissimilarities_allyears_nopollock.csv"))
